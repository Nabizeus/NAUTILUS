...
def visualize_forecast(truth, pred, title_prefix="Forecast", save_path=None):
    import matplotlib.pyplot as plt
    import numpy as np
    import os
    import torch
    ...
    # 🔒 Handle both torch tensors and numpy arrays safely
    ...
    truth_np = to_numpy_safe(truth)
    pred_np = to_numpy_safe(pred)



    # Plot
    fig, axs = plt.subplots(1, 2, figsize=(10, 4))
    im1 = axs[0].imshow(truth_np, cmap="viridis")
    axs[0].set_title(f"{title_prefix} - True")
    fig.colorbar(im1, ax=axs[0], shrink=0.7)

    im2 = axs[1].imshow(pred_np, cmap="viridis")
    axs[1].set_title(f"{title_prefix} - Predicted")
    fig.colorbar(im2, ax=axs[1], shrink=0.7)

    plt.tight_layout()
    out_path = f"artifacts/{title_prefix.replace(' ', '_')}_forecast.png"
    plt.savefig(out_path, dpi=200, bbox_inches="tight")
    plt.close()
    print(f"📸 Saved forecast comparison to: {out_path}")
