...
class WindowedOceanDataset(Dataset):
    ...
    def _plot_region_boxes(self, nc_path, regions=None):
        ...
        plt.pcolormesh(lon, lat, np.zeros_like(lon), cmap="Greys", alpha=0.15)
        plt.xlabel("Longitude")
        plt.ylabel("Latitude")
        plt.title("Defined Region Boxes (surface projection)")

        for reg in regions:
            name = reg.get("name", "region")
            # ✅ Support both YAML-style (x_range, y_range) and flattened x1/x2 format
            if "x_range" in reg and "y_range" in reg:
                x1, x2 = reg["x_range"]
                y1, y2 = reg["y_range"]
            else:
                x1, x2 = reg["x1"], reg["x2"]
            ...
            z_idx = reg.get("z_index", None)

            # Draw the box boundaries
            x_corners = [x1, x2, x2, x1, x1]
            y_corners = [y1, y1, y2, y2, y1]
            plt.plot(lon[y_corners, x_corners], lat[y_corners, x_corners], "-r", lw=2)

            # Annotate with label and index coordinates
            #label = f"{name} (z={z_idx})" if z_idx is not None else name
            #plt.text(
            #    lon[y1, x1], lat[y1, x1],
            #    f"{label}\n({x1},{y1})–({x2},{y2})",
            ...
            # Add approximate grid index ticks for reference
            ny, nx = lat.shape
            plt.xticks(np.linspace(lon.min(), lon.max(), 6),
                       [f"x={int(i)}" for i in np.linspace(0, nx, 6, dtype=int)])
            plt.yticks(np.linspace(lat.min(), lat.max(), 6),
                       [f"y={int(i)}" for i in np.linspace(0, ny, 6, dtype=int)])

            plt.tight_layout()
            out_path = f"artifacts/region_boxes_{name}.png"
            plt.savefig(out_path, dpi=150)
            plt.close()
            print(f"✅ Saved region visualization → {out_path}")
