...
def main():
    ...
    for i, (inputs, targets, mask) in enumerate(tqdm(loader, desc="Predicting")):
        ...
        if inputs.ndim == 6:
            inputs_2d = inputs[..., 10, :, :] #at 10.mean(dim=3)  # top 3 depth levels
        else:
            inputs_2d = inputs
        ...
        if i < 10:  # save first 10 examples
            ...
            if true_np.ndim == 3:
                true_np = np.nanmean(true_np[10:10, :, :], axis=0)  # average over depth
            if pred_np.ndim == 3:
                pred_np = np.nanmean(pred_np[10:10, :, :], axis=0)
